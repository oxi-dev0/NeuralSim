...
namespace Tools {
	...
	{
	public:
		Config(int sizeX_, int sizeY_, int tileSize_, int population_, int internalNeuronCount_, int genomeLength_, int mutationChance_, int stepsPerGen_, int gensBetweenRender_, std::string framerate_, std::string neuralmap_, int kconst_, std::string survival_) { sizeX = sizeX_; sizeY = sizeY_; tileSize = tileSize_; population = population_; internalNeuronCount = internalNeuronCount_; genomeLength = genomeLength_; mutationChance = mutationChance_; stepsPerGen = stepsPerGen_; gensBetweenRender = gensBetweenRender_; framerate = framerate_; neuralmap = neuralmap_; kconst = kconst_; survivalconfigfile = survival_; }
		Config() { sizeX = 128; sizeY = 128; tileSize = 6; population = 1000; internalNeuronCount = 2; mutationChance = 0.001f; genomeLength = 4; stepsPerGen = 350; gensBetweenRender = 100, framerate = "350/10"; neuralmap = ""; kconst = 400; survivalconfigfile = "config/survival.sv"; }
